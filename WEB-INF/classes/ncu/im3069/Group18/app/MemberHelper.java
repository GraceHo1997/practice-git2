//刪除會員(deleteByID)、取得所有會員資料(getAll)、取得會員資料(getByID)
//檢查該名會員之電子郵件信箱是否重複註冊(checkDuplicate)
//新增會員(create)、更新會員資料(update)
...
public class MemberHelper {
    ...
    public JSONObject getAll() {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                m = new Member(member_id, email, password, name, phone, identity);
    ...
    public JSONObject create(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "INSERT INTO `missa`.`members`(`name`, `email`, `password`, `phone`, `identity`)"
                    + " VALUES(?, ?, ?, ?, ?)";
    ...
    public JSONObject update(Member m) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `missa`.`members` SET `name` = ? ,`password` = ? , `phone` = ?  WHERE `email` = ?";
            /** 取得所需之參數 */
            String name = m.getName();
            String email = m.getEmail();
            String password = m.getPassword();
